...
public class Game {
	...
	// ########################################################################################################################
	// main game engine and "AI" control (simple AI it choose always a random token)
			...
			switch(result) {
				...
				case 4:
					...
					for(int i=0; i<result; i++) {
						// select
						Player p = this.selectVisiblePlayerMenu();
						if(p == null) {
						...
						p.setColor(this.selectColorMenu(p, result));
	...
	public void saveCurrentGame() {
		if(this.playerList.findPlayer(this.board.getScore().getActPlayer().getName()) != null) {
			...
			System.out.println("Game saved as: "+ this.board.getScore().getCreateTime());
			this.sleep(1500);
		} else {
		...
		do {
			...
			switch(result) {
				...
				case 1:
					...
					do {
						// select player
						p = this.selectVisiblePlayerMenu();
						if(p != null) {
							String newName = "";
							do {
								this.menu.plotHeader();
	...
	// ########################################################################################################################
	// additional functions to clean up the UI functions
	
	public ColorItem selectColorMenu(Player p, int amountPlayers) {
		int result = -1;
		...
		do {
			...
			default:
				...
				// color exist
				colorItem = colorList.get(result);
				break;
			}
	...
	public Player selectVisiblePlayerMenu() {
		int result = -1;
		do {
			result = this.menu.selectMenu(this.menu.selectPlayerMenu(this.playerList.getPlayerList()), "Select player");
			// back to menu
			if(result == 99) {
				return null;
			}
